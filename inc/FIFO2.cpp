...
bool Queue::IsEmpty(void){
  return ((PutI == GetI) && !full_q);  // replace this with solution
}

  // To check whether Queue is full or not
bool Queue::IsFull(void){
  return full_q;  // replace this with solution
}
...
bool Queue::Put(char x){
    ...
    full_q = (PutI==GetI); // only full when they are equal after push
...
bool Queue::Get(char *pt){
    ...
    *pt = Buf[GetI];
    ...
    full_q = false; // we remove an element, thus never empty
...
void Queue::Print(void){
  ...
  // output to ST7735R
    int FIFO_index = GetI;
    while(FIFO_index != PutI){
        ST7735_OutChar(Buf[FIFO_index]);
        FIFO_index = (FIFO_index + 1)%FIFOSIZE;
    }
}
